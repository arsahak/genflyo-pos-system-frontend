...
const EditSupplier = ({ supplierId }: EditSupplierProps) => {
  ...
  useEffect(() => {
    const fetchSupplier = async () => {
      try {
        const { getSupplierById } = await import("@/app/actions/suppliers");
        ...
        if (result.success && result.data) {
          setFormData({
            ...
            paymentTerms: result.data.paymentTerms || "30",
            notes: result.data.notes || "",
          });
        } else {
          toast.error(result.error || "Failed to fetch supplier details");
        }
        setFetching(false);
      } catch (error) {
      ...
      <div className={`min-h-screen p-6 transition-colors duration-300 ${
        ...
        <div className="max-w-[1920px] mx-auto">
          <div className="animate-pulse space-y-6">
            <div className={`h-8 w-48 rounded ${isDarkMode ? "bg-gray-700" : "bg-gray-200"}`} />
            <div className={`h-12 w-64 rounded ${isDarkMode ? "bg-gray-700" : "bg-gray-200"}`} />
            <div className={`h-96 rounded-2xl ${isDarkMode ? "bg-gray-800" : "bg-white"}`} />
          </div>
        </div>
      </div>
    );
    ...
    <div className={`min-h-screen p-6 transition-colors duration-300 ${
      ...
      <div className="max-w-[1920px] mx-auto">
        ...
        <div className="mb-6">
          ...
          >
            {getTranslation("editSupplier", language)}
          </h1>
          <p className={isDarkMode ? "text-gray-400" : "text-gray-600"}>
            {getTranslation("updateSupplierProfile", language)}
          </p>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-3">
              ...
              <button
                type="submit"
                disabled={loading}
                className="flex-1 py-3 px-6 rounded-xl font-semibold text-white bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 transition-all shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
                  ...
                  <>
                    ...
                    {getTranslation("updateSupplier", language)}
                  </>
                )}
              </button>
            </div>
